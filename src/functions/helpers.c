...
void set_dec_sign(s21_decimal *dst, int sign) {
...
int bigdec_to_decimal(s21_bigdecimal src, s21_decimal *dst) {
  ...
  *dst = (s21_decimal){0};
  ...
  for (int i = 3; i < 7; i++) {
    if (src.bits[i] & 0xFFFFFFF) error = CONVERT_ERROR;
  }
  return error;
}
...
int is_dec_zero(s21_decimal src) {
  int i = 95;
  while (i >= 0 && !get_dec_bit(src, i)) i--;
  return i == -1;
}
...
int is_greater_or_equal_bigdec(s21_bigdecimal value_1, s21_bigdecimal value_2) {
  return !is_less_bigdec(value_1, value_2);
}
...
void scale_checker(s21_bigdecimal *src) {
  int scale = get_bigdec_scale(*src);
  if (scale > 28) {
